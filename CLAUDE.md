...
本プロジェクトはいったん事前学習に特化します。理由：

1. **スコープの明確化** - 生成機能とKVキャッシュを含めると複雑性が増大
2. **YAGNI原則** - 現時点で必要ない機能は実装しない
3. **Early Exitとの相性** - 生成時のKVキャッシュ管理はEarly Exitと相性が悪い（研究でも品質低下が報告されている）
4. **段階的拡張** - 将来、生成機能が必要になれば別モジュールとして追加可能
...
生成機能を追加する場合は以下を検討：
- KVキャッシュをレイヤー内部で保持する設計（外部渡しではなく）
- LayerSkipのようなSelf-Speculative Decoding手法の採用
