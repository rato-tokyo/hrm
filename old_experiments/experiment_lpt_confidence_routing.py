"""
Experiment: LPT + Confidence-Routed Transformer
...
def train_epoch_lpt(
    ...
    for x, y in batches:
        ...
        # Sum all losses (equal weight)
        loss = l1_loss + l2_loss + l3_loss + l4_loss
...
def train_epoch_lpt_routing(
    ...
    for x, y in batches:
        ...
        loss = 0.5 * l1_loss + 1.0 * l2_loss + 0.5 * l3_loss + 1.0 * l4_loss
...
def main():
    ...
    print("=" * 70)
    print("Experiment: LPT + Confidence-Routed Transformer")
    print("=" * 70)
    print(f"\nConfig:")
    ...
    # ===== 2. Full LPT (L1 + L2 + L3 + L4) =====
    print("\n" + "=" * 70)
    print("2. Full LPT (L1 + L2 + L3 + L4 Loss)")
    print("=" * 70)
    ...
    models = [
        ...
        ('Full LPT', model_lpt),
        ('LPT + Routing Aware', model_lpt_routing)
    ]
    ...
    print(f"\n{'Model':<25} {'PPL':>8} {'Acc':>8} {'Shallow%':>10} {'Compute%':>10}")
    print("-" * 65)
    for r in results:
        print(f"{r['model']:<25} {r['ppl']:>8.2f} {r['accuracy']*100:>7.1f}% "
    ...
    print(f"\n2. Standard vs LPT + Routing Aware:")
