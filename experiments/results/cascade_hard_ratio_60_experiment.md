...
## Ensemble評価結果（全トークンでの評価）

**評価日時**: 2025-12-17 10:02

### 結果サマリー

| パターン | 段階数 | パラメータ | Val PPL | Exit率 |
|----------|--------|-----------|---------|--------|
| **Base only** | 0 | 134.5M | **7.89** | 100% (Base) |
| Base + Stage1 | 1 | 191.1M | 41.83 | Base: 40.7%, Stage1: 59.3% |
| Base + Stage1-2 | 2 | 247.8M | 51.06 | Base: 40.7%, S1: 26.5%, S2: 32.8% |
| Base + Stage1-3 | 3 | 304.4M | 54.92 | Base: 40.7%, S1: 26.5%, S2: 14.8%, S3: 18.0% |
| Base + Stage1-4 | 4 | 361.0M | 56.61 | Base: 40.7%, S1: 26.5%, S2: 14.8%, S3: 8.0%, S4: 10.1% |
| Base + Stage1-5 | 5 | 417.7M | 57.02 | Base: 40.7%, S1: 26.5%, S2: 14.8%, S3: 8.0%, S4: 4.5%, S5: 5.5% |

### 推奨パターン

**Base only (val_ppl=7.89)** が最良。

### 重大な発見

**追加したStage LLMは全く効果がなく、むしろ性能を悪化させている。**

```
Base only:      val_ppl = 7.89   (最良)
Base + Stage1:  val_ppl = 41.83  (5.3倍悪化)
Base + 全Stage: val_ppl = 57.02  (7.2倍悪化)
```

### 原因分析

1. **ベースLLMが既に優秀**
   - SmolLM2-135Mは2Tトークンで事前学習済み
   - val_ppl=7.89は十分に良い性能

2. **Stage LLMの能力不足**
   - 新規作成（スクラッチ）で少量データのみで訓練
   - 56.6Mパラメータでは134.5Mの事前学習モデルに勝てない

3. **Early Exit設計の問題**
   - 閾値0.123は低すぎ（40%しかexitしない）
   - 60%のトークンを未熟なStage LLMが予測 → 性能悪化

4. **訓練-評価のミスマッチ**
   - 訓練: hard tokensのみ
   - 評価: 全トークン（Early Exit後の残りトークン）
   - Stage LLMは見たことないトークンパターンを予測させられる

### 教訓

**現在のCASCADE実装は、事前学習済みベースモデルに対して効果がない。**

改善が必要な点:
1. Stage LLMも事前学習済みモデルを使用
2. Stage LLMをベースから蒸留/初期化
3. Early Exit判定指標の再設計
4. アーキテクチャの根本的見直し

### 結論

この実験により、「小さい新規LLMをhard tokensで訓練してベースに追加する」
アプローチは、事前学習済みベースモデルに対しては**有効ではない**ことが判明した。

ベースラインとの比較（SmolLM2-360Mファインチューニング）を実施し、
今後のアプローチを再検討する必要がある。
